...
class PositionTypeImport implements ToCollection, SkipsEmptyRows, WithStartRow, WithHeadingRow, WithValidation
{
    use Importable;
    ...
    public function rules(): array
    {
        return [
            ...
            '*.nama_position_type' => 'required|string',
        ];
    }
    ...
    public function customValidationAttributes()
    {
        return [
            '*.id' => 'Id',
            '*.code_position_type' => 'Code PositionType',
            '*.nama_position_type' => 'Nama PositionType',
        ];
    }
    ...
    public function collection(Collection $collection)
    {
        ...
        if (
            $data_import['id'] == 'Contoh ID Position Type' &&
            $data_import['code_position_type'] == 'Contoh Code Position Type' &&
            $data_import['nama_position_type'] == 'Contoh Nama Position Type'
        ...
        foreach ($collection as $value) {
            $client = RefPositionType::find($value['id']);

            if ($client) {
                $client->code_position_type    = $value['code_position_type'];
                $client->nama_position_type    = $value['nama_position_type'];
                $client->updated_at     = date('Y-m-d H:i:s');
                $client->created_by     = auth()->user()->username;

                $client->save();
            }else{
                RefPositionType::create([
                    'code_position_type'      => $value['code_position_type'],
                    'nama_position_type'      => $value['nama_position_type'],
